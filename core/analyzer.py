...
Currently uses keyword matching + templates (no external API).
To switch to LLM: replace the body of `analyze()` — the signature stays unchanged.
"""
...
from core.scoring import calculate_score, get_recommendation
...
def analyze(vacancy_text: str, profile: CandidateProfile) -> VacancyAnalysis:
    ...
    Placeholder implementation: keyword scoring + template responses.
    Replace this function body with an LLM call when ready —
    the signature and return type must remain unchanged.
    """
    score, matches, risks = calculate_score(vacancy_text, profile)
    recommendation = get_recommendation(score)

    role_hint = _extract_role_hint(vacancy_text, profile.target_roles)
...
# Keep threshold visible for tests without importing scoring directly
